...
namespace TSP
{
    ...
    class ProblemAndSolver
    {
        ...
        {
            string[] results = new string[3];
            double[][] costTable = createCostTable();
        ...
        {
            double[][] costTable = new double[Cities.Length][];

            //set table to costs
            for (int i = 0; i < Cities.Length; i++)
            {
                costTable[i] = new double[Cities.Length];
                for (int j = 0; j < Cities.Length; i++)
                {
                    costTable[i][j] = Cities[i].costToGetTo(Cities[j]);
                }
            }
            return costTable;
        }
